...
/**
 * Factory that returns a legacy (`experimentalDecorators`) method decorator for recording
 * call durations.
 *
 ...
 * @example
 * ```ts
 * const LegacyHistogramMetric = createLegacyHistogramMetric({ transport: myTransport });
 *
 * class OrderService {
 *   \@LegacyHistogramMetric()
...
export const createLegacyHistogramMetric =
    ...
    <TArgs extends readonly unknown[]>(config?: HistogramOptionsInterface<TArgs>): LegacyMethodDecoratorType => {
        ...
        return createLegacyInterceptor<TArgs, unknown>({
            interceptor: {
