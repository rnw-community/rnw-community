...
import type { PaymentAddress } from '../interface/payment-address';
import type { PaymentDetailsModifier } from '../interface/payment-details-modifier';
import type { PaymentDetailsUpdate } from '../interface/payment-details-update';
import type { PaymentRequestUpdateEventInit } from '../interface/payment-request-update-event-init';
...
export default class PaymentRequestUpdateEvent {
    ...
    _handleDetailsChange: (details: PaymentDetailsModifier) => Promise<any>;
    _resetEvent: any;

    constructor(readonly name: string, private readonly target: PaymentRequestUpdateEventInit) {
    ...
    async updateWith(
        PaymentDetailsModifierOrPromise:
            ...
            | ((modifier: PaymentDetailsModifier, address: PaymentAddress) => Promise<PaymentDetailsUpdate>)
    ) {
         ...
         */
        emptyFn(); // NativePayments does this for us (iOS does at the time of this comment)
        ...
        if (typeof PaymentDetailsModifierOrPromise === 'object') {
            ...
            return await this._handleDetailsChange(paymentDetailsModifier);
        }
