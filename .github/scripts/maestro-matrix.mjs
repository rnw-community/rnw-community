...
const EXAMPLES = [
    {
        ...
        targets: { bare: 'ReactNativePaymentsExample', expo: 'reactnativepaymentsexpoexample' },
        members: [
    ...
    },
    {
        ...
        targets: { expo: 'reactnativecollapsibleheaderexpoexample' },
        members: [
...
const isAffected = example => affectedNames === null || example.members.some(name => affectedNames.has(name));

const include = EXAMPLES.filter(isAffected).flatMap(example =>
    Object.entries(example.targets).map(([target, iosScheme]) => ({
